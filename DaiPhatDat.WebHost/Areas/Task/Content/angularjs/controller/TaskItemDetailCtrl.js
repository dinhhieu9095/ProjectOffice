    ...
    $scope.taskItem = {
        ...
        getData: function () {
            ...
            TaskItemDetailService.getData($scope.taskItem.filter.TaskItemId).then(function (rs) {
                ...
                $scope.taskItem.countAttachTask = $scope.taskItem.item.Attachments? $scope.taskItem.item.Attachments.filter(e => e.Source == 1).length : 0;
                $scope.taskItem.countAttachProcess = $scope.taskItem.item.Attachments? $scope.taskItem.item.Attachments.filter(e => e.Source == 2).length : 0;
                $scope.taskItem.countAttachChild = $scope.taskItem.item.AttachmentChildren? $scope.taskItem.item.AttachmentChildren.length: 0;
