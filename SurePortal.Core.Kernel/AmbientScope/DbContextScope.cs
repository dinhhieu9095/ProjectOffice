...
namespace SurePortal.Core.Kernel.AmbientScope
{
    public class DbContextScope : IDbContextScope
    {
        ...
        public int SaveChanges()
        {
            ...
            if (_completed)
                throw new InvalidOperationException(@"
                    ...
                    A DbContextScope is meant to encapsulate a business transaction:
                    create the scope at the start of the business transaction
                    and then call SaveChanges() at the end.
                    Calling SaveChanges() mid-way through a business transaction doesn't make sense
                    and most likely mean that you should refactor your service method into
                    two separate service method that each create their own DbContextScope
            ...
            if (!_nested) changes = CommitInternal();
        ...
        public async Task<int> SaveChangesAsync(CancellationToken cancelToken)
        {
            ...
            if (!_nested) changes = await CommitInternalAsync(cancelToken).ConfigureAwait(false);
        ...
        public void RefreshEntitiesInParentScope(IEnumerable entities)
        {
            ...
            {
                if (!(_parentScope._dbContexts.InitializedDbContexts.Values
                        .SingleOrDefault(parentContext => parentContext.GetType() == contextInCurrentScope.GetType())
                    is IObjectContextAdapter correspondingParentContext))
                    continue;

                // Both our scope and the parent scope have an instance of the same DbContext type.
                // We can now look in the parent DbContext instance for entities that need to be refreshed.
                foreach (var toRefresh in entityList)
                {
                    ...
                    if (stateInParentScope.State == EntityState.Unchanged)
                        correspondingParentContext.ObjectContext.Refresh(RefreshMode.StoreWins
                            , stateInParentScope.Entity);
                }
            }
        }
        ...
        public async Task RefreshEntitiesInParentScopeAsync(IEnumerable entities)
        {
            ...
            foreach (IObjectContextAdapter contextInCurrentScope in _dbContexts.InitializedDbContexts.Values)
            {
                ...
                foreach (var toRefresh in entityList)
                {
                    ...
                    if (stateInParentScope.State == EntityState.Unchanged)
                        await correspondingParentContext.ObjectContext
                            .RefreshAsync(RefreshMode.StoreWins, stateInParentScope.Entity)
        ...
        public void Dispose()
        {
            ...
            if (_parentScope != null)
            {
                if (_parentScope._disposed)
                {
                    ...
                    var message = @"
                        PROGRAMMING ERROR - When attempting to dispose a DbContextScope,
                        we found that our parent DbContextScope has already been disposed!
                        This means that someone started a parallel flow of execution
                        (e.g. created a TPL task, created a thread or enqueued a work item on the ThreadPool)
                        ...
                        You can do this with IDbContextScopeFactory.SuppressAmbientContext()
                        (wrap the parallel task creation code block in this).
         ...
         * a TheadStatic-based ambient DbContext here:
         * http://coding.abel.nu/2012/10/make-the-dbcontext-ambient-with-unitofworkscope/
         *
        ...
        private static readonly ConditionalWeakTable<InstanceIdentifier, DbContextScope>
            DbContextScopeInstances = new ConditionalWeakTable<InstanceIdentifier, DbContextScope>();
        ...
        internal static DbContextScope GetAmbientScope()
        {
            ...
            if (DbContextScopeInstances.TryGetValue(instanceIdentifier, out var ambientScope)) return ambientScope;
            ...
            Debug.WriteLine(@"
                ...
                Found a reference to an ambient DbContextScope in the CallContext
                but didn't have an instance for it in our DbContextScopeInstances table.
                This most likely means that this DbContextScope instance wasn't disposed of properly.
                DbContextScope instance must always be disposed.
                Review the code for any DbContextScope instance used outside of a 'using' block
                and fix it so that all DbContextScope instances are disposed of.");
